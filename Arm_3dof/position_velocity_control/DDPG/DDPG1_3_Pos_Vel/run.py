...
parser.add_argument('--env', default='Arm-v0',
    ...
    if args.mode == 'test':
        ...
        for i in range(args.MAX_EPISODES_TEST):
            ...
            for j in range(args.MAX_EP_STEPS):
                ...
                action1[2] = action[1] + joint_current[1]
                action1[4] = action[2] + joint_current[2]
                ...
                if (done == True) or (j == args.MAX_EP_STEPS - 1):
                    # print(j)
                    print('Episode:', i, "done: ", done, ' Reward: %i' % int(ep_reward), 'Explore: %.2f' % args.explore_noise, )
                    break
        ...
        for i in range(args.MAX_EPISODES_TRAIN):
            ...
            for j in range(args.MAX_EP_STEPS):
                ...
                if agent.pointer > args.MEMORY_CAPACITY:
                    args.explore_noise *= 0.9999  # decay the action randomness
                    agent.learn()

                s = s_
                ep_reward += r
                if done or j == args.MAX_EP_STEPS - 1:
                    print('Episode:', i, ' Reward: %i' % int(ep_reward), 'Explore: %.2f' % args.explore_noise, "done:%s" % done)
                    if ep_reward > -300:
                        RENDER = True
                    break
